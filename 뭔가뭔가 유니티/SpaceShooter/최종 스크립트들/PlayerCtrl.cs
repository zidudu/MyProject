...
public class PlayerCtrl : MonoBehaviour
{
    ...
    private float slidingColliderHeight = 0.5f;
    ...
    //bool isTimeCooldown = false;
    //float slowDuration = 5.0f;   // 슬로우 지속 시간
    //float slowCooldown = 10.0f;  // 슬로우 쿨타임
    ...
    //MonsterCtrl[] monsters;


    //// 플레이어 애니메이션 (Animation 컴포넌트)
    //Animation playerAnimation;



    public float slowRadius = 500.0f; // 슬로우 모션 범위
    public LayerMask monsterLayer;  // 몬스터 레이어
    public float slowMultiplier = 0.5f; // 속도 감소 배율
    public float slowDuration = 5.0f;   // 지속 시간
    public float slowCooldown = 10.0f;  // 쿨타임
    private bool isSlowMotion = false;  // 슬로우 모션 활성화 여부
    ...
    bool isDead = false; // 플레이어 사망 상태 확인 변수 추가
    ...
    IEnumerator Start()
    {
        ...
        //// 씬 내 몬스터들 참조
        //monsters = FindObjectsOfType<MonsterCtrl>();
    ...
    void Update()
    {
        ...
        // 플레이어가 죽었다면 움직임, 점프, 대쉬 등 모든 입력 처리 중단
        if (isDead)
        {
            return;
        }
        ...
        {
            currentSpeed = runSpeed; //달리기 속도

            if (currDash > 0) // 대시 값이 남아있는 경우
            {

                //currentSpeed = moveSpeed * 1.5f; // 대시 속도
        ...
        // T 키로 시간조작 시도
            ...
            if (Input.GetKeyDown(KeyCode.T) && !isSlowMotion && !isTimeCooldown)
            {
                GameManager.instance.ActivateTimeManipulation();
        ...
        {
            //aiming = true;
            //StartCoroutine(Aiming(h, v));
            // 수평/수직 이동값을 0으로 고정해 움직이지 못하게 함
            h = 0f;
            v = 0f;
    ...
    IEnumerator Aiming(float h, float v) { 
    yield return new WaitForSeconds(0.5f);
    ...
    IEnumerator SlowMotionRoutine()
    {
        ...
        // 슬로우 모션 시작 - 플레이어 속도 조정
        float targetSpeedMultiplier = 0.5f; // 속도를 50%로 감소
        AdjustPlayerSpeed(targetSpeedMultiplier);

        // 주변 몬스터 감지 및 속도 감소
        //ApplySlowMotionToNearbyMonsters(slowMultiplier); // 0.5f
        ApplySlowMotionToNearbyMonsters(slowMultiplier);
        ...
        //{
        //    // 주변 몬스터를 계속 감지 및 속도 감소 적용
        //    ApplySlowMotionToNearbyMonsters(slowMultiplier);

        //    elapsedTime += Time.deltaTime;
        //    yield return null; // 다음 프레임까지 대기 (매 프레임 반복)
        //}

        /*
    public float slowRadius = 10.0f; // 슬로우 모션 범위
    public LayerMask monsterLayer;  // 몬스터 레이어
    public float slowMultiplier = 0.5f; // 속도 감소 배율
    public float slowDuration = 5.0f;   // 지속 시간
    public float slowCooldown = 10.0f;  // 쿨타임
    ...
    private bool isTimeCooldown = false;
         ...
         */
        // 슬로우 모션 지속 시간
        yield return new WaitForSeconds(slowDuration); //5초

        // 원래 속도로 복구
        ApplySlowMotionToNearbyMonsters(1.0f);
        // 원래 속도로 복구
        AdjustPlayerSpeed(1.0f);
        isSlowMotion = false;
    ...
    void ApplySlowMotionToNearbyMonsters(float multiplier)
    {
        // 플레이어 주변의 몬스터 감지
        Collider[] hitColliders = Physics.OverlapSphere(transform.position, slowRadius, monsterLayer);

        foreach (var hitCollider in hitColliders)
        {
            var monster = hitCollider.GetComponent<MonsterCtrl>();
            if (monster != null)
            {
    ...
    void AdjustPlayerSpeed(float multiplier)
    {
        moveSpeed = originalMoveSpeed * multiplier;
        runSpeed = originalRunSpeed * multiplier;
        turnSpeed = originalTurnSpeed * multiplier;
    ...
    void PlayerDie()
    {
        ...
        //주인공 사망하ㅐㅆ을때 더는 적캐릭 생성x. gamemanager에서 싱글턴을 이용해 instance를 받아 Isgameover를 true로 바꿈.
    ...
    public void PlayerAnim(float h, float v, string KEY_STATE)
    {
        ...
        if (KEY_STATE == "DASH" && currDash > 0)
        {
            ...
            {
                anim.CrossFade("Idle Rifle", 0.18f); // 앞으로 달리기
            }
        }
        //걷기
        ...
        {
            anim.CrossFade("Death_Rifle", 0.25f); // 앞으로 달리기
